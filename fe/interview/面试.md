...
- 客户端使用https url访问服务器，则要求web服务器建立ssl链接。（ssl： security sockets layer）
...
* get请求会把请求报文一次性发出去，post请求会分为两个数据包
* get请求是安全且幂等的，它是只读的，post请求会修改数据，是不安全，且非幂等的
...
页面被加载时，link会同时被加载。@import引用的css会等到页面加载完毕后加载
...
Redux 有三大原则： **单一数据源：** flux的数据源可以是多个。 **Store中的状态是只读的**，只能通过派发action，修改状态，每次修改状态否会返回一个新的state，而flux的Stotre中的状态可以随意修改。 **只允许使用纯函数来修改**， flux执行修改不一定是纯函数。
...
60. dom diff的整体思路：
    ...
    3. 若比较到最后，新旧dom树均有剩余，则进入同层比较阶段。如果新的节点在旧树中找不到，则插在oldStartNode的后面，如果在旧树中找的到，则原地复用，移动到oldStartNode的后面。
    4. 比较完之后，拿最新的virtual dom更新视图。

61. Map,Set,weakMap, weakSet

**Map和Set的区别**

Map和Set很大程度上是相似的，到那时map是以键值对的方式存储数据的。
...
react fiber把更新过程碎片化，执行过程如下图所示，每执行一段更新过程，就把控制权交还给react负责任务协调的莫亏啊，看看有没有其他紧急任务要做，如果没有就继续更新，如果有紧急任务，那就去做紧急任务。

fiber，英文含义是纤维，意🈯指比thread更细的线，也就是比线程（Thread）控制的更精密的并发处理机制。

需要注意的是fiber方式的更新过程可能被打断，所以React Fiber一个更新过程会被分为两个阶段（phase）：第一个阶段 Reconciliation Phase和第二阶段Commit Phase。

在第一个阶段 Reconciliation Phase， React Fiber会找出需要更新哪些DOM，这个阶段是可以被打断的，但是到了第二阶段 Commit Phase，那就一鼓作气把DOM更新完，绝不会额比打断，

以render函数为界，第一阶段可能会调用下面这些生命周期函数，说是可能会调用是因为不同生命周期调用的函数不同。
...
第一阶段的过程会被打断且“重头再来”，会造成意想不到的情况。

比如说，一个低优先级的任务A正在执行，已经调用了某个组件的componentWillUpdate函数，接下来发现自己的时间分片已经用完了，于是冒出水面，看看有没有紧急任务，这个时候真的又一个紧急任务B，接下来react fiber就会执行这个紧急任务B，任务A虽然进行了一一半，但是没办法，只能选择完全放弃，等到任务B全搞定之后，任务Ac红来一遍，注意，重点是重头来一遍，不是从刚才中断的部分开始，也就是说，componentWillUpdate函数会被再调用一次，所以才有我们的数据请求之类的逻辑处理操作尽量不要放在 componentWillUpdate 或者 componentWillMount 生命周期里，因为可能会被多次调用。

在现有的react中，每个生命周期函数在一个加载或者更新过程中绝对会被调用一次，在react fiber中，不再是这样了，第一阶段的生命周期函数在一次加载和更新过程中会被多次调用。