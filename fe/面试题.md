...
存储型是攻击者输入一些数据并存储到了数据库中，其他浏览这看到的时候进行攻击，反射型的话不存储在数据库中，往往表现为将攻击代码放在url参数中，来实现攻击。
...
* es5中可以通过 Object.defineProperty 来实现已有属性的监听
...
利用有src属性的标签没有跨域限制的漏洞，需要服务器支持，仅支持get请求，不安全，可能会遭遇xss攻击。
...
116. https是如何保证安全的? 是如何保证不被中间人攻击的?


117. 子组件是一个Portal，发生点击事件能冒泡到父组件吗？

Portals是一种将子组件渲染到DOM树中任意位置的技术。尽管Portal的DOM节点可以位于DOM树中的任何位置，但它的行为和普通React组件一样，仍然遵循React的事件冒泡机制。因此，Portal中的点击事件是可以冒泡到父组件的。

118. git merge 和 git rebase

  - git merge：将两个分支的历史记录合并，并生成一个新的合并提交；
  保留完整的历史记录，适合用于公共分支（如main、develop）的合并；
  - git rebase：将一个分支的提交“重新应用”到另一个分支上，形成线性的提交历史；
  重写提交历史，使历史记录更加简洁。适合用于个人分支或特性分支的合并。
...
```
  - ```flex-grow: 1```：子项可以扩展填充剩余空间；
  - ```flex-shrink: 1```：子项可以收缩以适应空间不足的情况；
  - ```flex-basis: 0```：子项在分配剩余空间之前的大小是0；

120. 关系型数据库和非关系型数据库的区别
  1. 数据模型：
    - 关系型数据库：使用表格结构，数据以行和列的形式存储，表之间通过外键关联；
    - 非关系型数据库：使用多种数据模型，如键值对、文档、列族、图等，数据存储更灵活；
  2. 查询语言：
    - 关系型数据库：使用SQL进行查询和操作；
    - 非关系型数据库：通常使用特定API或查询语言，不依赖SQL；
  3. 扩展性：
    - 关系型数据库：通常垂直扩展，增加单机性能；
    - 非关系型数据库：通常水平扩展，通过增加服务器数量提升性能；
  4. 事务支持：
    - 关系型数据库：支持ACID事务，保证数据一致性；
    - 非关系型数据库：部分支持ACID，但通常更注重性能和扩展性，可能牺牲一致性；
  5. 适用场景：
    - 关系型数据库：适合复杂查询和事务支持的场景，如金融系统；
    - 非关系型数据库：适合需要高扩展性和灵活数据模型的场景，如大数据、实时应用；

121. 常见的质量保障手段和理论基础：
  - 持续集成/持续交付（CI/CD）：持续集成和持续交付是一种自动化软件交付流程的方法，通过频繁的集成代码、自动化测试和部署，确保软件始终处于可发布状态；
  - 测试驱动开发（TDD，test-driven development）是一种开发方法，要求开发者在编写功能代码之前先编写测试用例。通过这种方式，确保代码在开发过程中始终符合预期，提高代码质量和可维护性；
  - 行为驱动开发（BDD，behavior-driven development）是一种协作开发方法，通过自然语言描述系统行为，确保开发团队、测试团队和业务团队对需求有共同的理解，BDD强调从用户的角度出发，编写可执行的测试用例；
  - DevOps：DevOps是一种文化和实践，旨在通过开发和运维团队之间的紧密协作，实现快速、可靠的软件交付。DevOps强调自动化、监控和反馈，确保软件在整个生命周期中保持高质量；
  - 质量保证（QA）自动化：质量保证工具和框架可以帮助开发团队自动化测试流程，确保软件在不同环境和配置下的稳定性和可靠性；

122. requestAnimationFrame 和 requestIdleCallback 的区别

**requestAnimationFrame：**
  - 目标：为动画和视觉提供一个高效的机制；
  - 特点：
    - 执行时机：requestAnimationFrame 会在下一次重绘（repaint）之前调用回调函数。通常每16.6ms一次；
    - 用途：适用于需要动画效果、视觉过渡等场景，使动画更加流畅；
    - 效率：浏览器会根据刷新频率优化执行，避免不必要的重复绘制，从而提升性能；
  - 适合场景：适合需要每帧更新的高帧率动画；

**requestIdleCallback：**
  - 目标：在浏览器空闲时执行低优先级、非关键性的任务；
  - 特点：
    - 执行时机：requestIdleCallback会在浏览器空闲时（即不再忙于处理高优先级任务和渲染任务时）调用回调函数；
    - 用途：适用于执行非关键性任务，例如分析计算、日志记录、预加载数据等，避免阻塞主线程，影响用户体验；
    - 效率：通过在空闲时间执行任务，避免影响关键渲染路径，减少卡顿现象；
  - 适合场景：适合可以延后执行，不影响用户体验的任务；

结合两者可以实现更好的性能，如在动画过程中结合requestIdleCallback处理一些低优先级的任务。

123. js实现取消请求的几种方式

  1. 使用AbortController；
  AbortController是现代浏览器提供的一种原生API，用于取消fetch请求；
  ```js
  ...
  XMLHttpRequest是传统的AJAX请求方式，它提供了abort方法用于取消请求。
  ```js
  ...
  ```
  4. react query的取消功能
  react query内置了对取消请求的支持，可以通过signal或cancel方法实现。
  ```js
...
124. webpack频繁触发编译或者热更的原因
  1. 文件监听模式配置问题
  webpack默认会监听文件变化并触发重新编译。如果配置不当，可能会导致频繁触发编译。
    - 文件监听范围过大：webpack监听了不必要的文件或目录；
    - 文件变动频繁：某些工具（如IDE、文件同步工具）频繁修改文件；
  2. 热更新（HMR）配置问题
  热更新是webpack的一项功能，允许在不刷新页面的情况下更新模块。如果配置不当，可能会导致频繁更新；
    - HMR插件未正确配置：缺少HotModuleReplacementPlugin或配置错误；
    - 依赖模块频繁变化：某些模块（如css、图片）频繁更新；
  3. 插件或loader问题
  某些插件或loader可能会导致webpack频繁触发编译。
    - 插件或loader配置错误：如file-loader、url-loader等配置不当；
    - 插件或loader的副作用：某些插件可能会修改文件或触发重新编译；
  4. 依赖模块问题
  某些依赖模块可能会导致webpack频繁触发编译
    - 依赖模块频繁更新：如node_modules中的模块被频繁修改；
    - 依赖模块包含动态加载逻辑：如动态import或require.context；
...
版本格式：主版本号.次版本号.修订号，版本号递增规则如下：
  ...
  3. 修订号：当你做了向下兼容的问题修正；
先行版本号及编译版本信息可以加到“主版本号.次版本号.修订号”的后面，作为延伸。

126. 白屏时间的统计和优化
  - 白屏时间的统计
    1. 使用performance api；
    ```js
    // 通常将用户开始访问到FP（First Paint）这段时间视为白屏时间
    // 用户开始访问web页面的时间点到FCP（First Contentful Paint）的这段时间视为无内容时间
    const timing = window.performance.timing;
    const paintEntries = performance.getEntriesByType('paint');
    const firstPaint = paintEntries.find(entry => entry.name === 'first-paint');
    const time = firstPaint - timing.navigationStart;
    console.log('白屏时间：'， time, 'ms');
    ```
  - 白屏时间的优化
    1. 减少关键资源加载时间；
      - 压缩html、css、js等文件资源；
    ...
    2. 优化关键渲染路径；
      - 减少css的加载，异步加载非关键css；
      - 延迟加载js资源：使用async或defer属性异步加载js；
    3. 减少主线程工作量；
      - 优化js执行：避免长任务，将复杂计算拆分成多个小任务；
      - 使用web workers：将耗时的计算任务移至web workers，避免阻塞主线程；
    4. 预加载关键资源：
      - preload，高优先级加载首屏需要的关键资源；
      - prefetch，空闲时预取后续页面所需的资源；
    5. 优化服务器响应时间
      - 启用HTTP/2：通过多路复用和头部压缩提升性能；
      - 使用缓存：设置合理的缓存策略，减少重复请求；
    6. 使用服务端渲染：
      - SSR框架：使用Next.js等服务端渲染框架，提前生成HTML内容；
      - 静态站点生成（SSG）：对于内容固定的页面，使用SSG生成静态HTML；

127. 市面上流行的微前端方案有哪些，优劣势分别是什么？
