...
1. babel 的处理过程总共分为三个阶段， 解析，转换，生成。
  ...
  ![parse ast](<./images/whiteboard_exported_image (4).png>)
    - 词法分析：将源代码进行分词，拆分成一系列的Token，每个token都是一个最小的语法单元，例如关键字、标识符、运算符、数字、字符串等；
    - 语法分析：语法分析时解析过程的第二步，它的任务是根据词法分析生成的一系列token，构建抽象语法树；
  ...
  sourcemap记录了源码到目标代码的映射关系，通过它我们可以找到目标代码中每一个节点对应的源码位置，用于调试的时候把编译后的代码映射回源码，或者线上报错的时候把报错位置映射到源码。
